...
def Amaral_Model(c1, c2, l, n, v, time_limit):         
    ...
    constant_1 = gp.quicksum(c1[i+1, j+1]*(l[i] + l[j]) for i in range(0,n-1) for j in range(i+1,n))/2
    objective_1 = gp.quicksum(c1[i+1, j+1] * l[k] * x[i,k,j] for i in range(0,n-1) for j in range(i+1,n) for k in range(n) if (k != i and k != j))
